...
export default function MomInsights({moodEntries,journalEntries,babyActivities,moodTimeView,setMoodTimeView,trendTimeframe,setTrendTimeframe}) {
    const getTrendDays = () => {
    ...
    const moodTimeData = moodTimeView === 'day'
        ? [
            ...
            const moods = moodEntries.filter(
                m => m.time && group.hours.includes(parseInt(m.time.split(':')[0]))
            );
            const avgMood = moods.length > 0
                ? Math.round(moods.reduce((sum, m) => sum + m.mood_value, 0) / moods.length)
                : null;
    ...
    const daysWithJournal = new Set(
        journalEntries.map(j => format(new Date(j.created_date), 'yyyy-MM-dd'))
    );
    ...
    const avgMoodWithJournal = moodWithJournal.length > 0
        ? Math.round(moodWithJournal.reduce((sum, m) => sum + m.mood_value, 0) / moodWithJournal.length)
        : null;

    const avgMoodWithoutJournal = moodWithoutJournal.length > 0
        ? Math.round(moodWithoutJournal.reduce((sum, m) => sum + m.mood_value, 0) / moodWithoutJournal.length)
        : null;
